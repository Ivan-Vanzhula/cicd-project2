...
from django.shortcuts import render, get_object_or_404, \
    redirect  # Импортируем функции для рендеринга шаблонов и получения объектов
from django.core.exceptions import \
    ObjectDoesNotExist  # Импортируем исключение для работы с объектами, которые могут не существовать
...
def add_cart(request, product_id):
    ...
    try:
        ...
        cart_item = CartItem.objects.create(product=product, quantity=1,
                                            cart=cart)  # Создаем элемент корзины, если его нет
        cart_item.save()
